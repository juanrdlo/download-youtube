...
class YouTubeDownloaderApp:
	...
	def _build_ui(self) -> None:
		...
		ttk.Label(top_frame, text="Buscar:").grid(row=0, column=0, padx=(0, 8), sticky="w")
		self.search_input_frame = tk.Frame(
			top_frame,
			bg="#FFFFFF",
		...
		self.search_underline = tk.Frame(top_frame, height=2, bg="#D0D7DE")
		...
		self.search_btn = tk.Button(
			...
			activeforeground="#0A3B74",
			disabledforeground="#9AAFCB",
			relief="flat",
			bd=0,
			highlightthickness=1,
			highlightbackground="#9EC3F5",
			highlightcolor="#9EC3F5",
			padx=16,
			pady=6,
			cursor="hand2",
		)
		self.search_btn.bind("<Enter>", self._on_search_btn_enter)
		self.search_btn.bind("<Leave>", self._on_search_btn_leave)
		self.search_btn.grid(row=0, column=2, padx=(8, 0))

		self.max_results_var = tk.IntVar(value=20)
		ttk.Label(top_frame, text="Resultados:").grid(row=0, column=3, padx=(14, 6), sticky="e")
		...
		middle_frame = ttk.Frame(self.root, padding=(12, 0, 12, 0))
		...
		ttk.Label(header, text="Check", anchor="center").grid(row=0, column=0, sticky="ew")
		ttk.Label(header, text="Miniatura", anchor="center").grid(row=0, column=1, sticky="ew")
		ttk.Label(header, text="Titulo", anchor="w").grid(row=0, column=2, sticky="ew")
		ttk.Label(header, text="Duracion", anchor="center").grid(row=0, column=3, sticky="ew")
		ttk.Label(header, text="Canal", anchor="w").grid(row=0, column=4, sticky="ew")

		body = ttk.Frame(results_frame)
		...
		self.y_scroll = ttk.Scrollbar(body, orient="vertical", command=self.results_canvas.yview)
		self.y_scroll.grid(row=0, column=1, sticky="ns")
		self.results_canvas.configure(yscrollcommand=self.y_scroll.set)

		self.rows_frame = ttk.Frame(self.results_canvas)
		...
		selected_frame = ttk.Frame(middle_frame, padding=(12, 0, 0, 0))
		...
		ttk.Label(selected_frame, textvariable=self.selected_count_var).grid(row=0, column=0, sticky="w")
		...
		selected_scroll = ttk.Scrollbar(selected_frame, orient="vertical", command=self.selected_text.yview)
		...
		ttk.Label(bottom_frame, text=help_text, foreground="#333").grid(
		...
		ttk.Label(bottom_frame, textvariable=self.status_var).grid(row=1, column=1, padx=12, sticky="w")
	...
	def _on_search_btn_enter(self, _event=None) -> None:
		if str(self.search_btn.cget("state")) != "disabled":
			self.search_btn.configure(bg="#DDEBFF", highlightbackground="#79B0F1")

	def _on_search_btn_leave(self, _event=None) -> None:
		if str(self.search_btn.cget("state")) != "disabled":
			self.search_btn.configure(bg="#EAF3FF", highlightbackground="#9EC3F5")

	def _on_search_focus_in(self, _event=None) -> None:
		self.search_input_frame.configure(highlightbackground="#0A66D1")
		self.search_underline.configure(bg="#0A66D1")

	def _on_search_focus_out(self, _event=None) -> None:
		self.search_input_frame.configure(highlightbackground="#D0D7DE")
		self.search_underline.configure(bg="#D0D7DE")
	...
	def _render_load_more_button(self) -> None:
		...
		if not self.has_more_results:
			return
		row = tk.Frame(self.rows_frame, bg="#ffffff", bd=0, highlightthickness=0)
	...
	def _render_results(self, results) -> None:
		...
		for idx, item in enumerate(results):
			...
			thumb_lbl = tk.Label(row, image=self.placeholder_thumb, bg="#ffffff", width=100, height=60)
